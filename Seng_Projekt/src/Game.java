...
public class Game {
	...
	public Game(int mode, Minesweeper menu) {
		...
		if (gl.getMode() == 0 || gl.getMode() == 1 || gl.getMode() == 2) {
			gl.setDimensionY(gl.getMode() == 0 ? 8 : gl.getMode() == 1 ? 16 : 30); // Dimensionen
																					// der
																					// Spielfeldgröße
																					// je
																					// nach
		...
		if (gl.getMode() == 3 || gl.getMode() == 4 || gl.getMode() == 5) {
			gl.setDimensionY(gl.getMode() == 3 ? 8 : gl.getMode() == 4 ? 16 : 30); // Dimensionen
																					// der
																					// Spielfeldgröße
																					// je
																					// nach
																					// Schwierigkeitsgrad.
			gl.setDimensionX(gl.getMode() == 3 ? 8 : gl.getMode() == 4 ? 16 : 16); // Hier
																					// für
																					// Treasure
																					// Hunt
		...
		if (gl.getMode() == 6 || gl.getMode() == 7 || gl.getMode() == 8) {
			gl.setDimensionY(gl.getMode() == 6 ? 8 : gl.getMode() == 7 ? 16 : 30); // Dimensionen
																					// der
																					// Spielfeldgröße
																					// je
																					// nach
		...
		btnMenu = new JButton("zur\u00FCck zum Hauptmen\u00FC"); // Führt zurück
																	// zum
																	// Hauptmenü.
	...
	public JPanel addCells() { // Methode fügt die Zellen hinzu
		JPanel panel = new JPanel(new GridLayout(gl.getDimensionX(), gl.getDimensionY())); // panel
																							// =
																							// Hier
																							// werden
																							// die
																							// Zellen
																							// reingesetzt.
		panel2 = new JPanel(new GridLayout(gl.getDimensionX(), gl.getDimensionY())); // panel2
																						// =
																						// hinter
																						// den
																						// Zellen
		...
		switch (gl.getMode()) { // Je nach gewähltem Schwierigkeitsgrad (mode)
								// werden die Position und Skalierung des
								// Spielfeldes gesetzt.
		case 0:
		case 3:
		case 6:
		...
		flag.setImage(flag.getImage().getScaledInstance(width / gl.getDimensionY() - 5, height / gl.getDimensionX() - 5,
				Image.SCALE_DEFAULT)); // Größe der Flagge, die in einem Button
										// angezeigt wird. Abhängig von der
										// Größe des Spielfeldes.
		mine.setImage(mine.getImage().getScaledInstance(width / gl.getDimensionY() - 5, height / gl.getDimensionX() - 5,
				Image.SCALE_DEFAULT)); // Größe der Miene, die in einem Button
										// angezeigt wird. Abhängig von der
										// Größe des Spielfeldes.

		cells = new Cell[gl.getDimensionX()][gl.getDimensionY()]; // Array
																	// "cells"
																	// wird
																	// erstellt
																	// mit den
		...
		for (int i = 0; i < gl.getDimensionX(); i++) {
			for (int j = 0; j < gl.getDimensionY(); j++) {
				cells[i][j] = new Cell(this, new int[] { i, j }); // Cell-Objekte
																	// werden
																	// erstellt,
																	// Game-Objekt,
																	// Index der
		...
		if (gl.getMode() != 3 && gl.getMode() != 4 && gl.getMode() != 5) {
			spreadBombs();
		} // Mienen werden verteilt je nach gewähltem Schwierigkeitsgrad.
		else {
			spreadTreasure();
		}
	...
	private void spreadTreasure() {
		...
		if (gl.getMode() == 3 || gl.getMode() == 4 || gl.getMode() == 5) { // Schatz
																			// mit
																			// umgebenden
																			// Bomben
																			// setzen
			...
			for (int j = 0; j < bombs - k; j++) {
				...
				} while (cells[x][y].isBomb()); // Mienen werden zufällig auf
												// die Felder verteilt.
				...
				if (x != 0 && y != 0)
					cells[x - 1][y - 1].setValue(cells[x - 1][y - 1].getValue() + 1);
				// Die Werte (value) der Nachbarzellen einer Zelle mit Bombe
				// werden um 1 erhöht.
			}
		}
	}
